...
var Sort = (function(){
	...
	sort.alphanumeric = function(a,b) {             
        ...
        sort.time = function(a,b) {
            a = sort.trimValue(a);
            b = sort.trimValue(b);
            var match = a.match(/^(\d+)-(\d+)-(\d+) (\d+)\:(\d+)\:(\d+).(\d+)$/)
            var dateA = "";
            if (match == null){
                dateA = sort.gettime(a);
            }else{
                dateA = new Date(match[1], match[2] - 1, match[3], match[4], match[5], match[6]).getTime();
            }
            var match = b.match(/^(\d+)-(\d+)-(\d+) (\d+)\:(\d+)\:(\d+).(\d+)$/)
            var dateB = "";
            if (match == null){
                dateB = sort.gettime(b);
            }else{
                dateB = new Date(match[1], match[2] - 1, match[3], match[4], match[5], match[6]).getTime();
            }
            if (dateA==dateB) {
                return 0;
            } else if (dateA<dateB) {
        ...
        sort.gettime = function(a){
            a = sort.trimValue(a);
            var match = a.match(/^(\d+)-(\d+)-(\d+) (\d+)\:(\d+)\:(\d+)$/);
            var dateA = "";
            if (match == null){
                dateA = new Date(2500, 11, 31, 0, 0, 0).getTime();
            }else{
                dateA = new Date(match[1], match[2] - 1, match[3], match[4], match[5], match[6]).getTime();
            }
            return dateA;
        ...
        sort.timenoms = function(a,b) {
            a = sort.trimValue(a);
            b = sort.trimValue(b);
            var match = a.match(/^(\d+)-(\d+)-(\d+) (\d+)\:(\d+)\:(\d+)$/)
            var dateA = new Date(match[1], match[2] - 1, match[3], match[4], match[5], match[6]).getTime();
            var match = b.match(/^(\d+)-(\d+)-(\d+) (\d+)\:(\d+)\:(\d+)$/)
            var dateB = new Date(match[1], match[2] - 1, match[3], match[4], match[5], match[6]).getTime();
            if (dateA==dateB) {
                return 0;
            } else if (dateA<dateB) {
                return -1;
	...
	sort.date.convert = function(val) {
        ...
        sort.IP = function(a , b){
		//var aa = sort.makeStandardIPAddress(sort.getInnerText(a.cells[sort.sortColumnIndex]).toLowerCase());
		//var bb = sort.makeStandardIPAddress(sort.getInnerText(b.cells[sort.sortColumnIndex]).toLowerCase());
                a = sort.trimValue(a);
                b = sort.trimValue(b);
                var aa = sort.makeStandardIPAddress(a);
                var bb = sort.makeStandardIPAddress(b);
		if (aa==bb) {
			return 0;
		} else if (aa<bb) {
			return -1;
        ...
        sort.makeStandardIPAddress = function(val) {
                var vals = new Array();
                vals = val.split('.');
                for(valsIndex = 0; valsIndex<vals.length; valsIndex++){
                     val = vals[valsIndex];
                     while (3 > val.length) {
			val = '0'+val;
	             }
						...
						if (!hideRow) {
							...
							if (filter && col<cellsLength) {
								...
								if (filter.regex && val.search) {
                                                                    ...
                                                                    /*filterStr = filter.source;
                                                                    filterStr = filterStr.substring(1, filterStr.length);
                                                                    for(i=0; i<filterStr.length;i++){
                                                                        filChar = filterStr.charAt(i);
                                                                        valChar = '';
                                                                        if (val.length>i){
