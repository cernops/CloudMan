...
allUsers = getMembersInAGroup('myall-cern')
allGroups = []
count = 0
for userName in allUsers:
    ...
    oneUsersGroups = getGroupsFromMemberName(userName)
...
def getAllEgroups():
    returnValue = []
    filterString = '(OU=e-groups,OU=Workgroups,DC=cern,DC=ch)'
    print "%s" % filterString
    resultset  = pagedLDAPSearch(base='OU=Users,OU=Organic Units,DC=cern,DC=ch', \
        filter= filterString, attributes=['name'])
    ...
    if lengthResult == 0:
        print "No e-groups present...Strange"
...
#searchLDAPWithPaging("OU=Users,OU=Organic Units,DC=cern,DC=ch", "(memberOf:1.2.840.113556.1.4.1941:=CN=myall-cern,OU=e-groups,OU=Workgroups,DC=cern,DC=ch)" ,['name'])
#getAllEgroups();
