...
class ResourceForm(forms.Form):
    ...
    hepspecs = forms.FloatField(label="Capacity", help_text="Total Resources Capacity(HS06) -@@@@@@@@ If entered Positive Value greater than 0")
    memory = forms.FloatField(label="Memory",required=False, help_text="Total Available Memory in MB - Can be left blank")
    storage = forms.FloatField(label="Storage",required=False, help_text="Total Available Storage in MB - If entered Positive Value greater than 0")
...
class ResourceTypeForm(ResourceForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields.keyOrder = ['name', 'resource_class', 'hepspecs','memory','storage','bandwidth','comment']
...
class ZoneForm(forms.Form):
    def __init__(self, *args, **kwargs):
       #super(ResourceForm, self).__init__(*args, **kwargs)
       # self.fields.keyOrder = ['region','name', 'description', 'hepspec','memory','storage','bandwidth','comment']
        ...
        if isUserSuperUser:
            #region= forms.ModelChoiceField(queryset=Region.objects.all().values_list('name', flat=True))
            ...
            #region= forms.ModelChoiceField(queryset=Region.objects.filter(qset).values_list('name', flat=True))
            self.fields.insert(0, 'region', forms.ModelChoiceField(queryset=Region.objects.all().values_list('name', flat=True)))
...
class GroupAllocationForm(ResourceForm):
